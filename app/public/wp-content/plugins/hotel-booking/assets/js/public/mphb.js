...
MPHB.CheckoutForm = can.Control.extend( {
    ...
    getCountry: function () {
        var customerDetails = this.getCustomerDetailsOf(['country']);
        return customerDetails.country || false;
    },
    ...
    getCustomerDetailsOf: function (fields) {
        var details = {};
        var form = this.element;

        fields.forEach(function (field) {
            var fieldElement = form.find('#mphb_' + field);

            if (fieldElement.length > 0) {
                details[field] = fieldElement.val();
            }
        });
    ...
    getCustomerDetails: function () {
        ...
        $.extend(customer, this.getCustomerDetailsOf(['first_name', 'last_name', 'email', 'phone']));

        if (MPHB._data.settings.countryRequired) {
            $.extend(customer, this.getCustomerDetailsOf(['country']));
        }

        if (MPHB._data.settings.fullAddressRequired) {
            $.extend(customer, this.getCustomerDetailsOf(['country', 'address1', 'state', 'zip']));
        }
