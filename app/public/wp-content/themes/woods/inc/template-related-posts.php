...
function woods_related_posts() {
	...
	$visible = woods_theme()->customizer->get_value( 'related_posts_visible' );
	...
	$post_terms  = array();
	$post_number = woods_theme()->customizer->get_value( 'related_posts_count' );
	...
	foreach ( $settings as $setting_key => $setting_value ) {
		$settings[ $setting_key ] = woods_theme()->customizer->get_value( $setting_value );
	}
	...
	if ( $holder_view_dir ) {
		...
		foreach ( $posts as $post ) {
			...
			$image = ( $settings['image_visible'] ) ? woods_post_thumbnail( 'woods-thumb-s', array( 'echo' => false ) ) : '';
			...
			$author = ( $settings['author_visible'] ) ? woods_posted_by( array( 'echo' => false ) ) : '';

			$date = ( $settings['date_visible'] ) ? woods_posted_on( array( 'echo' => false ) ) : '';
