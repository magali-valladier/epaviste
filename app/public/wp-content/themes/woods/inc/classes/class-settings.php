...
if ( ! class_exists( 'Woods_Settings' ) ) {

	/**
	 * Define Woods_Settings class
	 */
	class Woods_Settings {
		 ...
		 * @var string
		 */
		public $key = 'woods-extra-settings';
		 ...
		 * @var string
		 */
		private $ajax_action = 'woods_save_theme_settings';
		...
		public function is_enabled() {
			return apply_filters( 'woods-theme/settings-page/is-enabled', true );
		}
		...
		public function admin_enqueue_scripts() {
			...
			if ( isset( $_REQUEST['page'] ) && $this->key === $_REQUEST['page'] ) {
				...
				$module_data = woods_theme()->framework->get_included_module_data( 'cherry-x-vue-ui.php' );
				...
				wp_localize_script(
					'woods-admin-script',
					'WoodsSettingsPageConfig',
					apply_filters( 'woods-theme/admin/settings-page-config', $this->settings_page_config )
				);
			}
		}
		...
		public function register_page() {
			...
			add_menu_page(
				esc_html__( 'Theme', 'woods' ),
				esc_html__( 'Theme', 'woods' ),
				'manage_options',
				$this->key,
				array( $this, 'render_page' ),
				get_theme_file_uri( 'assets/images/woods-theme-icon.svg' ),
				100
			);
		}
		...
		public function generate_config_data() {
			...
			$this->settings_page_config = array(
				...
				'messages' => array(
					'saveSuccess' => esc_html__( 'Saved', 'woods' ),
					'saveError'   => esc_html__( 'Error', 'woods' ),
				),
				'settingsData' => array(
		...
		public function save_settings() {
			...
			if ( ! isset( $_REQUEST['action'] ) || $this->ajax_action !== $_REQUEST['action'] || empty( $_REQUEST['options'] ) ) {
				wp_send_json_error( array(
					'message' => esc_html__( 'Server Error', 'woods' )
				) );
				return;
			}
			...
			if ( ! current_user_can( 'manage_options' ) ) {
				wp_send_json_error( array(
					'message' => esc_html__( 'Sorry! You cannot change options.', 'woods' )
				) );
			...
			wp_send_json_success( array(
				'message' => esc_html__( 'Settings have been saved', 'woods' )
			) );
		}
		...
		public function get_post_types( $is_archive_list = false ) {
			...
			if ( $is_archive_list ) {
				$result['search_results'] = esc_html__( 'Search Results', 'woods' );
				unset( $result['page'] );
			} else {
				$result['404_page'] = esc_html__( '404 Page', 'woods' );
			}
		...
		public function get_single_post_templates() {
			$default_template = array( 'default' => apply_filters( 'default_page_template_title', esc_html__( 'Default Template', 'woods' ) ) );
		...
		public function get_available_modules() {
			...
			if (  null === $this->available_modules ) {
				...
				$disabled_modules        = apply_filters( 'woods-theme/disabled-modules', array() );

				$modules_labels_map = array(
					'woo' => 'WooCommerce',
				);

				foreach ( woods_get_allowed_modules() as $module => $childs ) {
...
/**
 * Returns instance of Woods_Settings
 *
 * @return object
 */
function woods_settings() {
	return Woods_Settings::get_instance();
}
